...
gbf <- eventReactive(input$update,{
  if (input$map_zoom < 8) {
    ...
    coords<- datInBounds() 
    occ_search(taxonKey=input$type, limit=input$limit, hasCoordinate=TRUE, hasGeospatialIssues=FALSE, basisOfRecord=input$obs_method,decimalLatitude=coords[1], decimalLongitude=coords[2], fields=c('species','basisOfRecord','decimalLatitude', 'decimalLongitude'), return='data')}
